...
class WatchlistTeseCase(unittest.TestCase):
    def setUp(self):
        # 更新配置
        app.config.update(
            TESTING = True, #开启测试模式
            SQLALCHEMY_DATABASE_URI = 'sqlite:///:memory:'#使用SQLite内存型数据库，不会干扰开发时使用的数据库文件
        )
        #创建数据库和表
        db.create_all()
        ...
        self.runner = app.test_cli_runner() #创建测试命令运行器,flask1.0版本后才新增test_cli_runner,将flask版本换程1.0.2就解决了
    def tearDown(self):
